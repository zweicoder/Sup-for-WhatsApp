...
gulp.task 'sign:osx64', ['build:osx64'], ->
  ...
  shelljs.exec 'codesign -v --verify ./build/supforwhatsapp/osx64/supforwhatsapp.app'
...
gulp.task 'pack:osx64', ['sign:osx64'], ->
  ...
  shelljs.rm '-f', './dist/supforwhatsapp.dmg' # appdmg fails if the dmg already exists
...
gulp.task 'pack:win32', ['build:win32'], ->
  shelljs.exec 'm akensis ./assets-windows/installer.nsi'
  ...
  ['deb', 'rpm'].forEach (target) ->
    gulp.task "pack:linux#{arch}:#{target}", ['build:linux' + arch], ->
      ...
      move_opt = gulp.src [
        ...
        './build/supforwhatsapp/linux' + arch + '/**'
      ]
...
gulp.task 'run:osx64', ['build:osx64'], ->
  shelljs.exec 'open ./build/supforwhatsapp/osx64/supforwhatsapp.app'
...
# reget package
  ...
  # uses gulp-filter
  manifestFilter = tasks.filter([ 'manifest.json' ])
  regularJsons = tasks.filter([ '!manifest.json' ])
  gulp.src([
    './bower.json'
    './package.json'
    './src/manifest.json'
  ]).pipe(tasks.bump(version: newVer)).pipe(manifestFilter).pipe(gulp.dest('./src')).pipe(manifestFilter.restore()).pipe(regularJsons).pipe gulp.dest('./')
